...
def get_courses():
    ...
    usd = raw_usd.get('iso')
    usd_buy = raw_usd.get('buy')
    usd_sale = raw_usd.get('sale')
    raw_eur = currency_body.find(code='978')
    eur = raw_eur.get('iso')
    eur_buy = raw_eur.get('buy')
    eur_sale = raw_eur.get('sale')
    raw_rub = currency_body.find(code='643')
    rub = raw_rub.get('iso')
    rub_buy = round(float(raw_rub.get('buy')), 3) * 100
    rub_sale = round(float(raw_rub.get('sale')), 3) * 100
